...
class TestCourses:
    ...
    def test_edit_course(self, create_course_page: CreateCoursePage, courses_page: CoursesPage, chromium_page: Page):
        # компоненты
        create_course_form_component = CreateCourseFormComponent(chromium_page)
        courses_view_component = CourseViewComponent(chromium_page)
        create_course_exercises_toolbar_view_component = CreateCourseExercisesToolbarViewComponent(chromium_page)
        course_view_menu_component = CourseViewMenuComponent(chromium_page)
        image_upload_widget_component = ImageUploadWidgetComponent(chromium_page,identifier='create-course-preview')

        # создаём курс
        create_course_page.open("https://nikita-filonov.github.io/qa-automation-engineer-ui-course/#/courses/create")
        create_course_page.check_visible_create_course_title("Create course")
        create_course_page.check_disabled_create_course_button()
        create_course_page.check_visible_image_preview_empty_view("No image selected",
                                                                  "Preview of selected image will be displayed here")
        create_course_page.check_visible_image_upload_view('Tap on "Upload image" button to select file',
                                                           "Recommended file size 540X300")

        create_course_exercises_toolbar_view_component.check_visible("Exercises")
        create_course_page.check_visible_exercises_empty_view('There is no exercises',
                                                              'Click on "Create exercise" button to create new exercise')
        image_upload_widget_component.upload_preview_image("testdata/files/image.png")
        create_course_page.check_visible_image_remove_view('Tap on "Upload image" button to select file',
                                                           'Recommended file size 540X300')
        create_course_form_component.check_visible()
        create_course_form_component.fill(
        ...
        # редактируем курс
        course_view_menu_component.click_edit()
        create_course_form_component.fill(
            "Playwright_edited",
            "3 weeks",
            "Playwright_edited",
        ...
        courses_page.check_courses_title_is_visible()
        courses_page.check_visible_course_card()
        courses_view_component.check_visible(
            0,
            "Playwright_edited",
            "150",
            "100",
